<div class="ui sidebar vertical left menu overlay  borderless visible sidemenu inverted  grey"
    ...
    @if(Auth::user()->role===1)
        {{--menu item mobile--}}
        <a class="item {{request()->is('dashboard')? 'active':''}} popupHover" href="{{route('dashboard.index')}}"
           data-content="ផ្ទាំងគ្រប់គ្រង" data-position="right center">
            <i class="ion-speedometer icon"></i> <span class="colhidden">ផ្ទាំងគ្រប់គ្រង</span>
        </a>
        <div class="ui dropdown item displaynone scrolling popupHover" data-content="ទំនិញ"
        ...
        <a class="item popupHover {{request()->is('stock-import')? 'active':''}}" href="{{route('stock.import')}}"
           data-content="នាំទំនិញចូល" data-position="right center">
        ...
        <a class="item popupHover {{request()->is('media')? 'active':''}}" href="{{route('media.index')}}" href="#"
           data-content="មេឌៀ" data-position="right center">
        ...
        <div class="ui accordion inverted">
            ...
            <div class="content {{request()->is('report*')? 'active':''}}">
                <a class="item {{request()->is('report/import-export')? 'active':''}}"
                   href="{{route('report.import.export')}}">
                    លក់ចេញ / ទិញចូល - ថ្លៃឈ្នួល
                </a>
                <a class="item {{request()->is('report/income-expense')? 'active':''}}"
                   href="{{route('report.income.expense')}}">
                    ចំណូល / ចំណាយ - ថ្លៃឈ្នួល
                </a>
                <a class="item {{request()->is('report/import-export-no-income-note')? 'active':''}}"
                   href="{{route('report.import.export.no.income.note')}}">
                    លក់ចេញ / ទិញចូល
                </a>
                <a class="item {{request()->is('report/income-expense-no-income-note')? 'active':''}}"
                   href="{{route('report.income.expense.no.income.note')}}">
                    ចំណូល / ចំណាយ
                </a>
                <a class="item {{request()->is('report/income-note')? 'active':''}}"
                   href="{{route('report.income.note')}}">
                    ថ្លៃឈ្នួល
                </a>
                <a class="item {{request()->is('report/check-stock-index')? 'active':''}}"
                   href="{{route('report.check.stock.index')}}">
                    ពិនិត្យស្តុក
                </a>
            </div>
        ...
        {{--Dropdown mobile--}}
        <div class="ui dropdown item displaynone scrolling popupHover" data-content="អ្នកប្រើប្រាស់"
             data-position="right center">
        ...
        <div class="ui dropdown item displaynone scrolling popupHover" data-content="របាយការណ៍"
             data-position="right center">
            <span>របាយការណ៍</span>
        ...
        <a class="popupHover item {{request()->is('check-price')? 'active':''}}" href="{{route('report.check-price')}}" data-content="ពិនិត្យទំនិញ"
           data-position="right center">
            <i class="ion-cash icon"></i><span class="colhidden">ពិនិត្យទំនិញ</span>
        </a>
    @else
        {{--User--}}
        <a class="popupHover item {{request()->is('check-price')? 'active':''}}" href="{{route('report.check-price')}}" data-content="ពិនិត្យទំនិញ"
